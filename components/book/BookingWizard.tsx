...
import { paymentLinkForPrice } from "@/lib/stripe-links";
...
export function BookingWizard() {
  ...
  const payLink = paymentLinkForPrice(quote.price.id);
  ...
  async function finish() {
    ...
    try {
      ...
      const link = paymentLinkForPrice(quote.price.id);
      if (link) {
          ...
          <div className="space-y-4">
            ...
            {payLink ? (
              <p className="rounded-2xl bg-cream px-4 py-3 text-sm">
                Pay securely on Stripe. We still save your booking request first, then open the payment page.
              </p>
            ) : (
              <p className="rounded-2xl bg-cream px-4 py-3 text-sm">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              <button type="button" className="btn-primary" disabled={submitting} onClick={finish}>
                {submitting ? "Saving…" : payLink ? "Pay with Stripe" : "Submit request"}
              </button>
            </div>
          </div>
        )}
